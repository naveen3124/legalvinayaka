...
function sendingDataToJava($fields, $entry, $form_data, $entry_id) {
    ...
    // If form ID is not 22, return
    if (absint($form_data['id']) !== 22) {
        return;
    }

    // If form ID is 22, perform the Java integration
    if (absint($form_data['id']) == 22) {
        require_once("http://localhost:8087/JavaBridge/java/Java.inc");
        $searchKeyword = isset($fields[1]['value']) ? $fields[1]['value'] : 'None';
        //error_log(print_r($searchKeyword, true));

        $world = new java("com.lawman.core.Lawman_Main");
        $xml = new SimpleXMLElement('<?xml version="1.0" encoding="UTF-8"?><query></query>');
	    $xml->addChild('userInput', $searchKeyword);
	    $formatXML = $xml->asXML();
        $jsonData = java_values($world->hello($formatXML));
        session_start();
		
        $data = json_decode($jsonData, true);
        $_SESSION['query'] = $data;

        $url = get_template_directory_uri() . '/searchresults.php';
        //error_log(print_r($url, true));
        $page_url = get_permalink(72); // Replace PAGE_ID with the ID of the existing page

        // Redirect to the existing page with the custom template
...
function enqueue_custom_scripts() {
  ...
  wp_enqueue_script('custom-chat-script', get_template_directory_uri() . '/js/custom-chat-script.js', array('jquery'), '1.0', true);
  wp_localize_script('custom-chat-script', 'ajax_object', array('ajax_url' => admin_url('admin-ajax.php')));

}
add_action('wp_enqueue_scripts', 'enqueue_custom_scripts');

// custom-ajax-handler.php
function send_message_handler() {
  // Get the message from the AJAX request
  $message = $_POST['message'];
  // error_log(print_r($message, true));
  // Process the message (for demonstration purposes, we'll simply echo it back)
  echo 'naveen';
